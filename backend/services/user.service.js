...
export async function createOrUpdateUser(userData) {
  ...
  try {
    ...
    const query = `
      INSERT INTO users (sub, email, display_name, profile_picture, created_at, updated_at)
    ...
    const values = [sub, email, defaultDisplayName, defaultProfilePicture];
...
export async function getUserStats(sub) {
  ...
  try {
    const query = `
      ...
      WHERE u.sub = $1
      GROUP BY u.sub, u.wordhunt_games_played, u.timebomb_games_played, u.wordhunt_wins, u.timebomb_wins
    `;
...
/**
 * Update user profile information (display name and profile picture)
 * @param {string} sub - Auth0 user ID
 * @param {Object} profileData - Profile data to update
 * @param {string} [profileData.display_name] - New display name
 * @param {string} [profileData.profile_picture] - New profile picture URL
...
export async function updateUserProfile(sub, profileData) {
  ...
  const { display_name, profile_picture } = profileData;
